...
- 영어 텍스트를 다른 언어로 번역할 시스템을 어떻게 구축해야 할까요?
  <details markdown="1">
    ...
    > 5. Dockerize하여 container로 서비스한다.
...
- 뉴스 기사를 주제별로 자동 분류하는 시스템을 어떻게 구축할까요?
  <details markdown="1">
    ...
    > 1. 모델에 입력될 데이터와 결과 데이터를 pair하여 구성한다.  
    >    이 시스템의 경우 입력은 "기사 제목 텍스트"이 될 수도 있고, "기사 내용 텍스트"가 될 수도 있다.  
    > 2. 텍스트를 모델에 입력할 수 있는 형태로 변경하는 Embedding 방법을 선택한다.  
    > 3. NLP 모델을 선택하고 준비한 데이터를 이용하여 학습한다.  
    >    시계열 모델은 입력 데이터의 길이에 따라 선택한다. 비교적 짧은 길이의 기사 제목은 RNN 계열 모델로도 충분할 수 있고, 입력 길이가 긴 기사 내용의 경우, Transformer 기반 모델을 이용해야할 수도 있다. 
    > 4. 결과를 평가하고 필요 시, Keyword 기반 Machine Learning 모델과 앙상블한다.
    > 5. API 구성 후 Dockerize하여 서비스한다.
